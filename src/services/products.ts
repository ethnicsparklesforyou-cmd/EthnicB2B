...
export async function fetchProducts(filters: ProductFilters = {}) {
  ...
  queryParams.set('isB2b', 'true');
  const url = `${API_BASE_URL}products/fetch?${queryParams.toString()}`;
  const response = await fetch(url, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ page: 1, limit: 20, isB2b: true, ...filters }),
  });
  const data = await response.json();
  // Safe filtering: Ensure only B2B or isBoth products are returned
  if (data?.data?.products && Array.isArray(data.data.products)) {
    data.data.products = data.data.products.filter(
      (p: any) => p.isB2b === 1 || p.isB2b === true || p.isBoth === 1 || p.isBoth === true || (p.b2bPrice && parseFloat(p.b2bPrice) > 0)
    );
  }
  return data;
}
